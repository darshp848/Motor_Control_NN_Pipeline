...
$targetName = 'eesm_requal_direct_r9_02_solvediag_02'
...
$script = Join-Path $RepositoryRoot 'eesm\aedt\solve_direct_eesm_r9_02.py'
$outDir = Join-Path $RepositoryRoot 'out\eesm\task9_requalification_r9'
$preflight = Join-Path $outDir 'solve_preflight_attempt2.json'
$startedEvidence = Join-Path $outDir 'solve_started_attempt2.json'
$result = Join-Path $outDir 'solve_result_attempt2.json'
$runnerResult = Join-Path $outDir 'solve_runner_attempt2.json'
$manifest = Join-Path $outDir 'solve_results_manifest_attempt2.json'
$log = Join-Path $outDir 'solve_attempt2.log'
...
foreach ($path in @($targetDir, $preflight, $startedEvidence, $result, $runnerResult, $manifest, $log)) {
    if (Test-Path -LiteralPath $path) { throw "One-use r9 solve diagnostic already exists: $path" }
}
...
if (-not (Test-Path -LiteralPath $result)) {
    throw "AEDT exited without a solve result; inspect $runnerResult and $log"
}
$payload = Get-Content -Raw -LiteralPath $result | ConvertFrom-Json
$payload | ConvertTo-Json -Depth 8
if ($payload.status -ne 'solve_completed') {
    throw "r9 solve diagnostic returned status $($payload.status); inspect $result and $log"
}
