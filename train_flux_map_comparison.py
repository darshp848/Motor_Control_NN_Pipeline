...
def main(args):
    ...
    (Xtr, Ytr), (Xva, Yva), (Xte, Yte), tr_idx, va_idx, te_idx = \
        train_val_test_split(X_all, Y_all, seed=seed)
    ...
    with open(os.path.join(dirs["logs"], "metrics_summary.json"), "w") as f:
        # Keep backward-compatible list form for compare_off_grid_predictions.
        json.dump(results, f, indent=2)
    with open(os.path.join(dirs["logs"], "train_run_meta.json"), "w") as f:
        json.dump(meta, f, indent=2)

    # Pick in-grid winner (Stage 0: may be overridden after off-grid ranking).
    ...
    for r in ok:
        mark = "  <== in-grid winner" if r is winner else ""
        print(f"  {r['name']:<22} test_rmse_mean={r['test']['rmse_mean']:.5f} "
              f"r2_mean={r['test']['r2_mean']:.4f}{mark}")

    # Generate inference_flux_map.py for the in-grid winner; offline orchestrator
    # may re-promote to the off-grid in-domain winner via promote_inference_model.
...
def promote_inference_model(model_name, models_dir, metrics_results=None):
    """Rewrite inference_flux_map.py to load model_name from models_dir."""
    if metrics_results is None:
        summary = os.path.join(os.path.dirname(models_dir), "logs", "metrics_summary.json")
        with open(summary, "r") as f:
            metrics_results = json.load(f)
    winner = None
    for r in metrics_results:
        if r.get("name") == model_name and r.get("status") == "ok":
            winner = r
            break
    if winner is None:
        for ext in (".pkl", ".pt", ".npz"):
            pth = os.path.join(models_dir, model_name + ext)
            if os.path.exists(pth):
                winner = {"name": model_name, "status": "ok", "artifact_path": pth}
                break
    if winner is None:
        raise FileNotFoundError(
            "Cannot promote model %r: no metrics entry or artifact under %s"
            % (model_name, models_dir)
        )
    path = write_inference_file(winner, models_dir)
    print(f"[promote] inference_flux_map.py -> {model_name} ({winner.get('artifact_path')})")
    return path
